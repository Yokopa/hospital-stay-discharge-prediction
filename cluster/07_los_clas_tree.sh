...
#SBATCH --job-name=capped_los_7_classification
#SBATCH --output=/data/users/ascarpellini/hospital_stay_discharge_prediction/logs/07_capped_los_7_class_%A_%a.o
#SBATCH --error=/data/users/ascarpellini/hospital_stay_discharge_prediction/logs/07_capped_los_7_class_%A_%a.e
...
# RUN it with [7] and [4, 7] (with capped los, and whole los)

set -e
trap 'echo "Error occurred. Exiting script."; exit 1' ERR

echo "Starting LOS Classification | $(date)"
start_time=$(date +%s)
...
cd "$WORKDIR/src"
echo "Running LOS classification on dataset: $DATASET_NAME with model: $MODEL_NAME"
